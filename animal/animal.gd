...
var _last_dragged_position = Vector2.ZERO
...
func stoppped_rolling() -> bool:
	return get_contact_count() > 0 and abs(linear_velocity.y) < STOPPED and abs(angular_velocity) < STOPPED
	
func check_on_target() -> void:
	if not stoppped_rolling():
		return
...
func play_collision() -> void:
	if _last_collision_count == 0 and get_contact_count() > 0 and not collision_sound.playing:
